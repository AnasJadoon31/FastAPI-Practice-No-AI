...
class Teacher(SQLModel, table=True):
    ...
    degree: list[str] = Field(description="Degree List", sa_column=Column(JSON))
    marital_status: Enum_Marital_Status = Field(
        description="Marital Status of the teacher"
    )


class ModelTeacherUpdate(SQLModel):
    name: str | None = Field(
    ...
    experience_years: int | None = Field(
        default=None,
        description="Experience Years must be greater than or equal to 5",
        ge=5,
    )
    # degree: list[str] = Field(description="Degree List")
    degree: list[str] | None = Field(
        default=None, description="Degree List", sa_column=Column(JSON)
    )
    marital_status: Enum_Marital_Status | None = Field(
        default=None, description="Marital Status of the teacher"
    )
